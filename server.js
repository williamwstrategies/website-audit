...
app.post('/api/email/broadcast/seven-day-trial/test', async (req, res) => {
  try {
    requireEmailTestSecret(req);
    const to = singleEmailRecipient(req.body?.to || req.query.to || 'hallpwj@gmail.com');
    if (!to) {
    ...
    const baseStep = lifecycleEmails.sevenDayTrialBroadcastStep;
    const step = {
      ...baseStep,
    ...
    const delivery = await lifecycleEmails.sendLifecycleEmail({
      ...
      step,
      campaign: lifecycleEmails.TRIAL_BROADCAST_CAMPAIGN_KEY,
      ignorePause: true,
    });
...
app.all('/api/email/broadcast/lead-finder-launch/run', async (req, res) => {
  try {
    requireEmailTestSecret(req);
    const result = await lifecycleEmails.runLeadFinderLaunchBroadcast({
      dryRun: broadcastDryRun(req),
      limit: req.query.limit || req.body?.limit,
      confirm: req.query.confirm || req.body?.confirm,
    });
    posthog.capture({
      distinctId: 'lead-finder-launch-broadcast-runner',
      event: 'lead_finder_launch_broadcast_run',
      properties: {
        dry_run: result.dryRun,
        eligible: result.eligible,
        selected: result.selected,
