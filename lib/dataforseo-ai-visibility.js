...
function logProviderCompleted(result = {}) {
  console.log('[AI Visibility] provider request completed', {
    success: result.success === true,
    statusCode: result.statusCode || null,
    statusMessage: result.statusMessage || '',
    model: result.model || DEFAULT_MODEL_NAME,
    webSearch: result.webSearch === true,
    countryCode: result.countryCode || '',
    city: result.city || '',
    responseTextLength: cleanText(result.responseText, 12000).length,
    citations: asArray(result.citedUrls).length,
    fanOutQueries: asArray(result.fanOutQueries).length,
    cost: result.cost || 0,
    moneySpent: result.moneySpent || 0,
    durationMs: result.durationMs || 0,
    taskId: result.taskId || '',
  });
}

function logProviderFailed(result = {}) {
  console.log('[AI Visibility] provider request completed', {
    success: false,
...
async function runAiVisibilityPrompt(input = {}, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const result = providerError(`DataForSEO request failed with HTTP ${response.status}.`, 'dataforseo_http_error', {
      ...
      logProviderFailed(result);
      return result;
    }

    if (requestFailedByProvider(data)) {
      const result = providerError(providerFailureMessage(data), 'dataforseo_provider_error', {
    ...
    const normalized = normalizeDataForSeoResponse(data, { prompt, countryCode, city }, startedAt);
    logProviderCompleted(normalized);
    return normalized;
  } catch (error) {
