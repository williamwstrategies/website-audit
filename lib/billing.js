...
async function createReportForUser(userId, input = {}) {
  ...
  if (!canSaveWithSubscription && !canSaveComplimentaryReport) {
    throw httpError(
      402,
      subscription?.complimentary_scan_used_at
        ? 'Your complimentary assessment has been used. Choose a plan to continue generating reports.'
        : 'Run your complimentary assessment or choose a plan before saving reports.',
...
async function reserveAuditUsage(userId, idempotencyKey) {
  ...
  if (!normalized.can_run_audit) {
    ...
    const reason = !subscription
      ? 'no_subscription'
      : (subscription.complimentary_scan_used_at ? 'complimentary_scan_used' : (normalized.status === 'active' ? 'audit_limit_reached' : 'subscription_inactive'));
    return {
      allowed: false,
      reason,
