...
async function changeSubscriptionPlan(req, user, options = {}) {
  ...
  const subscription = await stripeRequest(`/subscriptions/${encodeURIComponent(existing.stripe_subscription_id)}`, {
    method: 'POST',
    params: {
      'items[0][id]': subscriptionItemId,
      'items[0][price]': stripePriceIdForPlan(selectedPlan.key),
      'metadata[plan]': selectedPlan.key,
  ...
  return {
    subscription: normalizeSubscriptionForClient(updated),
    planChange: updated?.__plan_change || (planRank(selectedPlan.key) > planRank(existing.plan) ? 'upgraded' : 'downgraded'),
    previousPlan: updated?.__previous_plan || existing.plan,
    currentPlan: updated?.__current_plan || selectedPlan.key,
  };
}
