...
const TRIAL_ACTIVATION_FEE_CENTS = 100;
const PLAN_CATALOG = {
  starter: {
    key: 'starter',
    name: 'Starter',
...
const REACTIVATION_EXPERIMENT_KEY = 'pitchproof_reactivation_2026_09';
const REACTIVATION_VARIANT_FREE_SCANS = 'reactivation_free_scans';
const REACTIVATION_VARIANT_DISCOUNT_MONTH = 'reactivation_discount_month';
const REACTIVATION_DISCOUNT_OFFER_KEY = 'reactivation_discount_month';
...
function publicPlan(plan) {
  const trialDays = plan.checkout ? subscriptionTrialDays() : 0;
  return {
    key: plan.key,
    name: plan.name,
    ...
    lead_discovery_allowance: plan.leadDiscoveryAllowance,
    ...
    trial_activation_fee_cents: trialDays > 0 ? TRIAL_ACTIVATION_FEE_CENTS : 0,
    trial_label: trialDays > 0 ? `${trialDays}-day trial + $1 today` : '',
...
function reactivationExperimentEnabled() {
  return /^(1|true|yes|on)$/i.test(cleanText(process.env.REACTIVATION_AB_TEST_ENABLED));
}

function reactivationDiscountPromotionCodeId() {
  return cleanText(
    process.env.STRIPE_REACTIVATION_10_PROMOTION_CODE_ID ||
    process.env.STRIPE_REACTIVATION_PROMOTION_CODE_ID
  );
}

function reactivationDiscountCouponId() {
  return cleanText(
    process.env.STRIPE_REACTIVATION_10_COUPON_ID ||
    process.env.STRIPE_REACTIVATION_COUPON_ID
  );
}

function reactivationDiscountCode() {
  return cleanText(
    process.env.STRIPE_REACTIVATION_10_PROMOTION_CODE ||
    process.env.STRIPE_REACTIVATION_PROMOTION_CODE ||
    process.env.STRIPE_REACTIVATION_10_CODE
  );
}

function reactivationDiscountConfigured() {
  return Boolean(reactivationDiscountPromotionCodeId() || reactivationDiscountCouponId() || reactivationDiscountCode());
}

function stripeIdKind(value = '') {
  const clean = cleanText(value);
  if (/^promo_/i.test(clean)) return 'promotion_code';
  if (/^coupon_/i.test(clean)) return 'coupon';
  return '';
}

function normalizeCheckoutOffer(value = '') {
  const clean = cleanText(value).toLowerCase();
  if (clean === REACTIVATION_DISCOUNT_OFFER_KEY || clean === 'reactivation' || clean === 'reactivation_10') {
    return REACTIVATION_DISCOUNT_OFFER_KEY;
  }
  return '';
}
...
async function findActivePromotionCodeId(code) {
  ...
  if (!cleanCode) return '';
  const result = await stripeRequest('/promotion_codes', {
    params: {
      code: cleanCode,
      active: 'true',
      limit: '1',
  ...
  return stringId(result?.data?.[0]?.id);
}

async function stripeCouponExists(couponId) {
  const cleanCouponId = cleanText(couponId);
  if (!cleanCouponId) return false;
  try {
    await stripeRequest(`/coupons/${encodeURIComponent(cleanCouponId)}`);
    return true;
  } catch (error) {
...
async function checkoutDiscountParamsFromValue(value, preferredType = '') {
  const clean = cleanText(value);
  if (!clean) return null;

  const kind = stripeIdKind(clean);
  if (kind === 'promotion_code') return { 'discounts[0][promotion_code]': clean };
  if (kind === 'coupon') return { 'discounts[0][coupon]': clean };

  if (preferredType === 'promotion_code') {
    const promotionCodeId = await findActivePromotionCodeId(clean);
    if (promotionCodeId) return { 'discounts[0][promotion_code]': promotionCodeId };
    throw httpError(
      503,
      `Stripe promotion code "${clean}" was configured, but no active promotion code was found. Use the Stripe promotion code ID that starts with promo_, or make sure the code is active.`,
      'stripe_abandoned_cart_discount_invalid'
    );
  }

  if (await stripeCouponExists(clean)) return { 'discounts[0][coupon]': clean };
  ...
  throw httpError(
    503,
    `Stripe discount "${clean}" was configured, but it was not found as a coupon ID or active promotion code.`,
    'stripe_abandoned_cart_discount_invalid'
  );
}

async function checkoutDiscountForOffer(offer, context = {}) {
  if (offer !== REACTIVATION_DISCOUNT_OFFER_KEY) return null;
  if (!reactivationExperimentEnabled()) {
    throw httpError(
      403,
      'This reactivation offer is not available right now.',
      'reactivation_ab_test_disabled'
    );
  }
  const enrollment = await getReactivationExperimentEnrollmentForUser(context.user?.id || context.userId);
  if (
    !enrollment ||
    enrollment.experiment_key !== REACTIVATION_EXPERIMENT_KEY ||
    enrollment.variant !== REACTIVATION_VARIANT_DISCOUNT_MONTH ||
    enrollment.converted_at ||
    enrollment.status === 'converted'
  ) {
    throw httpError(
      403,
      'This reactivation discount is not available for this account.',
      'reactivation_discount_unavailable'
    );
  }

  const configuredValue = reactivationDiscountPromotionCodeId() || reactivationDiscountCouponId() || reactivationDiscountCode();
  if (!configuredValue) {
    throw httpError(
      503,
      'The reactivation $10 first-month Stripe discount is not configured.',
      'reactivation_discount_missing'
    );
  }

  const preferredType = reactivationDiscountPromotionCodeId()
    ? 'promotion_code'
    : reactivationDiscountCouponId()
      ? 'coupon'
      : '';
  let params;
  try {
    params = await checkoutDiscountParamsFromValue(configuredValue, preferredType);
  } catch (error) {
    throw httpError(
      error?.statusCode || 503,
      error?.message || 'The reactivation Stripe discount could not be applied.',
      'reactivation_discount_invalid',
      error?.details || null
    );
  }
  return {
    offer: REACTIVATION_DISCOUNT_OFFER_KEY,
    label: '$10 first month',
    experiment_key: REACTIVATION_EXPERIMENT_KEY,
    variant: REACTIVATION_VARIANT_DISCOUNT_MONTH,
    params,
  };
}
...
function subscriptionTrialDays(options = {}) {
  ...
  if (options.skipTrial || options.startPaidNow || checkoutOffer === REACTIVATION_DISCOUNT_OFFER_KEY) return 0;
  return SUBSCRIPTION_TRIAL_DAYS;
}

function trialActivationFeeCents(trialDays) {
  return trialDays > 0 ? TRIAL_ACTIVATION_FEE_CENTS : 0;
}
...
function billingConfigStatus() {
  return {
    ...
    reactivationExperimentEnabled: reactivationExperimentEnabled(),
    reactivationDiscountConfigured: reactivationDiscountConfigured(),
    aiVisibilityEnabled: /^(1|true|yes|on)$/i.test(cleanText(process.env.AI_VISIBILITY_CUSTOMER_ENABLED)),
    plan: PROFESSIONAL_PLAN,
    plans: publicPlans(),
    trialDays: subscriptionTrialDays(),
    trialActivationFeeCents: TRIAL_ACTIVATION_FEE_CENTS,
  };
}
...
async function createReportForUser(userId, input = {}) {
  ...
  const canSavePromotionalReport = await canSaveWithReactivationPromotionalScan(userId, auditKey);

  if (!canSaveWithSubscription && !canSaveComplimentaryReport && !canSavePromotionalReport) {
    throw httpError(
      402,
      subscription?.complimentary_scan_used_at
  ...
  const promotionalCompletion = canSavePromotionalReport
    ? await completeReactivationPromotionalScan(userId, auditKey, {
        websiteUrl: report?.website_url,
        websiteScore: report?.website_score,
      }).catch(error => {
        console.warn('[PitchProof] Reactivation promotional scan completion failed:', error?.message || error);
        return null;
      })
    : null;
  const complimentaryCompletion = !canSavePromotionalReport
    ? await completeComplimentaryScan(
        userId,
        input.auditIdempotencyKey || input.audit_idempotency_key,
        report?.id
      ).catch(error => {
        console.warn('[PitchProof] Complimentary scan completion failed:', error?.message || error);
        return null;
      })
    : null;
  if (report && promotionalCompletion) report.reactivation_promotional_scan_completed = true;
...
function isReactivationSchemaMissing(error) {
  return error?.details?.code === '42P01' || /reactivation_/i.test(error?.message || '');
}

async function getReactivationExperimentEnrollmentForUser(userId) {
  ...
  try {
    const rows = await supabaseRest('reactivation_experiment_enrollments', {
      query: {
        ...
        experiment_key: `eq.${REACTIVATION_EXPERIMENT_KEY}`,
        limit: '1',
      },
    });
    return firstRow(rows);
  } catch (error) {
    if (isReactivationSchemaMissing(error) && !reactivationExperimentEnabled()) return null;
    throw error;
  }
}

async function getReactivationPromotionalGrantForUser(userId) {
  const cleanUserId = cleanText(userId);
  if (!cleanUserId) return null;
  try {
...
async function getReactivationExperimentStateForUser(userId) {
  if (!reactivationExperimentEnabled()) return null;
  const [enrollment, grant] = await Promise.all([
    getReactivationExperimentEnrollmentForUser(userId),
    getReactivationPromotionalGrantForUser(userId),
  ]);
  ...
  return {
    ...
    converted_at: enrollment.converted_at || null,
    promotional_scans_granted: grant?.credits_granted ?? 0,
    promotional_scans_remaining: grant?.credits_remaining ?? 0,
    promotional_scans_status: grant?.status || '',
  };
}

function applyReactivationStateToSubscription(normalized, state) {
  if (!state) return normalized;
  const promotionalScansRemaining = Math.max(0, Number(state.promotional_scans_remaining) || 0);
  const hasPromotionalScanAccess = (
    state.variant === REACTIVATION_VARIANT_FREE_SCANS &&
    promotionalScansRemaining > 0 &&
    !ACTIVE_STATUSES.has(normalized.status) &&
    !normalized.stripe_subscription_id
  );
  return {
    ...normalized,
    reactivation_experiment: state,
    reactivation_promotional_scan_access: hasPromotionalScanAccess,
    promotional_scans_remaining: promotionalScansRemaining,
    can_run_audit: normalized.can_run_audit || hasPromotionalScanAccess,
    can_save_reports: normalized.can_save_reports || hasPromotionalScanAccess,
  };
}
...
function canAttemptReactivationPromotionalScan(subscription, normalized) {
  if (!reactivationExperimentEnabled()) return false;
  if (subscription?.stripe_subscription_id) return false;
  return !ACTIVE_STATUSES.has(normalized?.status || normalizeStatus(subscription?.status || subscription?.subscription_status));
}

async function reserveReactivationPromotionalScan(userId, idempotencyKey) {
  if (!reactivationExperimentEnabled()) return null;
  try {
    const result = await supabaseRpc('reserve_reactivation_promotional_scan', {
      p_user_id: userId,
      p_experiment_key: REACTIVATION_EXPERIMENT_KEY,
...
async function completeReactivationPromotionalScan(userId, idempotencyKey, details = {}) {
  if (!reactivationExperimentEnabled() || !idempotencyKey) return null;
  try {
    const result = await supabaseRpc('complete_reactivation_promotional_scan', {
      p_user_id: userId,
...
async function refundReactivationPromotionalScan(userId, idempotencyKey) {
  if (!reactivationExperimentEnabled() || !idempotencyKey) return null;
  try {
    const result = await supabaseRpc('refund_reactivation_promotional_scan', {
      p_user_id: userId,
      p_idempotency_key: idempotencyKey,
    });
    return result?.refunded ? result : null;
  } catch (error) {
    if (isReactivationSchemaMissing(error) && !reactivationExperimentEnabled()) return null;
    throw error;
  }
...
async function canSaveWithReactivationPromotionalScan(userId, idempotencyKey) {
  const key = cleanText(idempotencyKey);
  if (!reactivationExperimentEnabled() || !key) return false;
  try {
    const rows = await supabaseRest('reactivation_promotional_credit_events', {
      query: {
        select: 'id,status',
        user_id: `eq.${userId}`,
    ...
    if (isReactivationSchemaMissing(error) && !reactivationExperimentEnabled()) return false;
    throw error;
  }
}
...
async function ensureSubscriptionForUser(userId) {
  ...
  if (existing) {
    ...
    const repaired = await patchSubscriptionForUser(userId, {
      ...
      console.warn('[PitchProof] Complimentary preview repair failed:', error?.message || error);
      return null;
    });
...
function normalizeSubscriptionForClient(subscription) {
  ...
  return {
    ...
    lead_discovery_allowance: plan.leadDiscoveryAllowance,
    leadDiscoveryAllowance: plan.leadDiscoveryAllowance,
...
async function getSubscriptionStatus(userId) {
  const subscription = await ensureSubscriptionForUser(userId);
  const normalized = normalizeSubscriptionForClient(subscription);
  const reactivationState = await getReactivationExperimentStateForUser(userId);
  return applyReactivationStateToSubscription(normalized, reactivationState);
}
...
async function createCheckoutSession(req, user, options = {}) {
  ...
  const checkoutOffer = normalizeCheckoutOffer(options.offer || options.checkoutOffer || options.discount || '');
  const selectedPlan = checkoutOffer === REACTIVATION_DISCOUNT_OFFER_KEY
    ? PLAN_CATALOG.professional
    : planForKey(options.plan || options.planKey || existing?.plan || DEFAULT_PLAN_KEY);
  ...
  const activationFeeCents = trialActivationFeeCents(trialDays);
  const checkoutDiscount = await checkoutDiscountForOffer(checkoutOffer, { user, selectedPlan, existing });
  const session = await stripeRequest('/checkout/sessions', {
    ...
    params: {
      ...
      ...(activationFeeCents > 0 && {
        ...
        'line_items[1][price_data][unit_amount]': activationFeeCents,
        'line_items[1][price_data][product_data][name]': 'PitchProof trial activation',
      ...
      ...(activationFeeCents > 0 && { 'metadata[trial_activation_fee_cents]': activationFeeCents }),
      ...(checkoutDiscount && {
        'metadata[checkout_offer]': checkoutDiscount.offer,
        'metadata[checkout_offer_label]': checkoutDiscount.label,
        'metadata[experiment_key]': checkoutDiscount.experiment_key,
        'metadata[experiment_variant]': checkoutDiscount.variant,
      }),
      ...
      ...(checkoutDiscount && {
        ...
        'subscription_data[metadata][experiment_key]': checkoutDiscount.experiment_key,
        'subscription_data[metadata][experiment_variant]': checkoutDiscount.variant,
      }),
      ...(trialDays > 0 && {
        payment_method_collection: 'always',
        'subscription_data[trial_period_days]': trialDays,
        'subscription_data[metadata][trial_days]': trialDays,
        'subscription_data[metadata][trial_activation_fee_cents]': activationFeeCents,
      }),
  ...
  await upsertSubscriptionForUser(user.id, {
    ...
    console.warn('[PitchProof] Checkout timestamp refresh failed:', error?.message || error);
  });

  if (checkoutDiscount?.offer === REACTIVATION_DISCOUNT_OFFER_KEY) {
    await markReactivationCheckoutStarted(user.id, {
      checkout_session_id: session.id,
      stripe_customer_id: customerId,
      plan: selectedPlan.key,
      checkout_offer: checkoutDiscount.offer,
    }).catch(error => {
      console.warn('[PitchProof] Reactivation checkout tracking failed:', error?.message || error);
    });
  }
...
async function patchReactivationEnrollmentById(enrollmentId, payload = {}) {
  const rows = await supabaseRest('reactivation_experiment_enrollments', {
    method: 'PATCH',
    prefer: 'return=representation',
    query: {
...
async function markReactivationCheckoutStarted(userId, metadata = {}) {
  if (!reactivationExperimentEnabled()) return null;
  const enrollment = await getReactivationExperimentEnrollmentForUser(userId);
  if (!enrollment || enrollment.variant !== REACTIVATION_VARIANT_DISCOUNT_MONTH) return null;
  const existingMetadata = metadataObject(enrollment.metadata);
  return patchReactivationEnrollmentById(enrollment.id, {
    activated_at: enrollment.activated_at || new Date().toISOString(),
    status: enrollment.status === 'assigned' ? 'activated' : enrollment.status,
    stripe_customer_id: metadata.stripe_customer_id || enrollment.stripe_customer_id || null,
    subscription_plan: metadata.plan || enrollment.subscription_plan || PROFESSIONAL_PLAN.key,
    metadata: {
      ...existingMetadata,
...
async function markReactivationConvertedFromCheckout(session, updatedSubscription) {
  if (!reactivationExperimentEnabled()) return null;
  if (session?.metadata?.checkout_offer !== REACTIVATION_DISCOUNT_OFFER_KEY) return null;
  const userId = cleanText(session.client_reference_id || session.metadata?.supabase_user_id || updatedSubscription?.user_id);
  const enrollment = await getReactivationExperimentEnrollmentForUser(userId);
  if (!enrollment || enrollment.variant !== REACTIVATION_VARIANT_DISCOUNT_MONTH) return null;
  const amountTotal = Number(session.amount_total ?? session.amount_subtotal);
  const existingMetadata = metadataObject(enrollment.metadata);
  return patchReactivationEnrollmentById(enrollment.id, {
    status: 'converted',
    activated_at: enrollment.activated_at || new Date().toISOString(),
    converted_at: enrollment.converted_at || new Date().toISOString(),
    subscription_id: stringId(session.subscription) || updatedSubscription?.stripe_subscription_id || enrollment.subscription_id || null,
    stripe_customer_id: stringId(session.customer) || updatedSubscription?.stripe_customer_id || enrollment.stripe_customer_id || null,
    subscription_plan: updatedSubscription?.plan || session.metadata?.plan || enrollment.subscription_plan || PROFESSIONAL_PLAN.key,
    first_payment_amount_cents: Number.isFinite(amountTotal) ? amountTotal : enrollment.first_payment_amount_cents,
    first_payment_at: enrollment.first_payment_at || (session.payment_status === 'paid' ? new Date().toISOString() : null),
    metadata: {
      ...existingMetadata,
      checkout_session_completed_at: new Date().toISOString(),
      checkout_session_id: stringId(session.id),
      checkout_payment_status: session.payment_status || '',
      checkout_offer: session.metadata?.checkout_offer || '',
      checkout_offer_label: session.metadata?.checkout_offer_label || '',
    },
  });
}

async function markReactivationInvoicePaid(invoice, updatedSubscription) {
  if (!reactivationExperimentEnabled()) return null;
  const subscriptionId = subscriptionIdFromInvoice(invoice) || updatedSubscription?.stripe_subscription_id;
  if (!subscriptionId) return null;
  const rows = await supabaseRest('reactivation_experiment_enrollments', {
    query: {
      select: '*',
  ...
  const enrollment = firstRow(rows);
  if (!enrollment) return null;
  const amountPaid = Number(invoice.amount_paid ?? invoice.amount_due);
  const invoiceCreatedAt = timestampToIso(invoice.created) || new Date().toISOString();
  const existingMetadata = metadataObject(enrollment.metadata);
  const payload = {
    metadata: {
      ...existingMetadata,
      last_invoice_paid_at: invoiceCreatedAt,
      last_invoice_id: stringId(invoice.id),
      last_invoice_amount_paid_cents: Number.isFinite(amountPaid) ? amountPaid : null,
      last_invoice_billing_reason: invoice.billing_reason || '',
    },
  };

  if (!enrollment.first_payment_at) {
    payload.first_payment_at = invoiceCreatedAt;
    if (Number.isFinite(amountPaid)) payload.first_payment_amount_cents = amountPaid;
  } else if (!enrollment.first_full_price_renewal_at && invoice.billing_reason === 'subscription_cycle') {
    payload.first_full_price_renewal_at = invoiceCreatedAt;
  }

  return patchReactivationEnrollmentById(enrollment.id, payload);
}
...
async function handleCheckoutCompleted(session) {
  ...
  const reactivationConversion = await markReactivationConvertedFromCheckout(session, updated).catch(error => {
    console.warn('[PitchProof] Reactivation checkout conversion tracking failed:', error?.message || error);
    return null;
  });
  if (updated && reactivationConversion) updated.__reactivation_conversion = reactivationConversion;
  return updated;
}
...
async function handleInvoiceEvent(invoice, paymentStatus) {
  ...
  if (subscriptionId) {
    ...
    if (paymentStatus === 'paid') {
      await markReactivationInvoicePaid(invoice, updated).catch(error => {
        console.warn('[PitchProof] Reactivation invoice tracking failed:', error?.message || error);
      });
    }
    return updated;
  }
...
async function handleStripeWebhook(rawBody, signatureHeader) {
  ...
  return {
    ...
    reactivationConversion: updatedSubscription?.__reactivation_conversion
      ? {
          experiment_key: REACTIVATION_EXPERIMENT_KEY,
          variant: updatedSubscription.__reactivation_conversion.variant || REACTIVATION_VARIANT_DISCOUNT_MONTH,
          enrollment_id: updatedSubscription.__reactivation_conversion.id || '',
        }
      : null,
  };
}
...
async function reserveAuditUsage(userId, idempotencyKey) {
  ...
  if (!normalized.can_run_audit) {
    if (canAttemptReactivationPromotionalScan(subscription, normalized)) {
      const reactivationState = await getReactivationExperimentStateForUser(userId);
      const promotionalReservation = await reserveReactivationPromotionalScan(userId, idempotencyKey);
      if (promotionalReservation) {
        const refreshedReactivationState = await getReactivationExperimentStateForUser(userId);
        return {
          ...promotionalReservation,
          subscription: applyReactivationStateToSubscription(normalized, refreshedReactivationState),
        };
      }
      if (reactivationState?.variant === REACTIVATION_VARIANT_FREE_SCANS) {
        return {
          allowed: false,
          reason: 'promotional_scans_exhausted',
          promotional: true,
          idempotency_key: cleanText(idempotencyKey) || crypto.randomUUID(),
          subscription: applyReactivationStateToSubscription(normalized, reactivationState),
        };
      }
    }
...
async function completeAuditUsage(userId, idempotencyKey, details = {}) {
  ...
  const promotionalCompletion = await completeReactivationPromotionalScan(userId, idempotencyKey, details);
  if (promotionalCompletion) return promotionalCompletion;
...
async function refundAuditUsage(userId, idempotencyKey) {
  if (!idempotencyKey) return null;
  const promotionalRefund = await refundReactivationPromotionalScan(userId, idempotencyKey).catch(error => {
    console.warn('[PitchProof] Reactivation promotional scan refund failed:', error?.message || error);
    return null;
  });
  if (promotionalRefund) return promotionalRefund;
  const complimentaryRefund = await refundComplimentaryScan(userId, idempotencyKey).catch(error => {
    console.warn('[PitchProof] Complimentary scan refund failed:', error?.message || error);
    return null;
  });
...
function publicError(error) {
  ...
  const safeServerCodes = new Set([
    ...
    'business_search_provider_payment_required',
...
module.exports = {
  ...
  publicPlans,
  publicError,
  reactivationDiscountConfigured,
