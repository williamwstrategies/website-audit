...
async function checkoutDiscountForOffer(offer) {
  ...
  return {
    ...
    params: promotionCodeId
      ? await checkoutDiscountParamsFromValue(promotionCodeId, 'promotion_code')
      : await checkoutDiscountParamsFromValue(couponId, 'coupon'),
  };
}
...
async function createCheckoutSession(req, user, options = {}) {
  ...
  const checkoutDiscount = await checkoutDiscountForOffer(checkoutOffer);
