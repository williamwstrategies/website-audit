...
async function upsertSubscriptionForUser(userId, payload = {}) {
  ...
  const body = {
    ...
    audit_limit: payload.audit_limit ?? existing?.audit_limit ?? PROFESSIONAL_PLAN.trialAuditLimit,
    ...payload,
  };
...
async function ensureSubscriptionForUser(userId) {
  ...
  const rows = await supabaseRest('subscriptions', {
    ...
    body: {
      ...
      plan: PROFESSIONAL_PLAN.key,
      status: 'incomplete',
      audit_limit: PROFESSIONAL_PLAN.trialAuditLimit,
      audits_used: 0,
    },
  });
  return firstRow(rows);
...
function auditLimitForStatus(status) {
  return normalizeStatus(status) === 'trialing'
    ? PROFESSIONAL_PLAN.trialAuditLimit
...
function normalizeSubscriptionForClient(subscription) {
  ...
  const defaultLimit = status === 'incomplete' ? PROFESSIONAL_PLAN.trialAuditLimit : auditLimitForStatus(status);
  const storedLimit = Math.max(0, Number(sub.audit_limit) || defaultLimit);
  const limit = status === 'trialing' ? PROFESSIONAL_PLAN.trialAuditLimit : storedLimit;
  ...
  return {
    ...
    trial_audit_limit: PROFESSIONAL_PLAN.trialAuditLimit,
    paid_audit_limit: PROFESSIONAL_PLAN.auditLimit,
    status,
...
async function createCheckoutSession(req, user, options = {}) {
  ...
  if (!customerId) {
    ...
    await upsertSubscriptionForUser(user.id, {
      ...
      audit_limit: grantTrial ? PROFESSIONAL_PLAN.trialAuditLimit : PROFESSIONAL_PLAN.auditLimit,
    });
  }
  ...
  const session = await stripeRequest('/checkout/sessions', {
    ...
    params: {
      ...
      ...(grantTrial && { 'subscription_data[trial_period_days]': String(PROFESSIONAL_PLAN.trialDays) }),
      'metadata[supabase_user_id]': user.id,
      'metadata[user_email]': user.email || '',
...
async function startPaidSubscriptionNow(req, user) {
  requireStripeConfig();
  await assertAuthUserExistsForWrites(user);
  ...
  if (!existing?.stripe_subscription_id) {
    return createCheckoutSession(req, user, { skipTrial: true, startPaidNow: true });
  }
...
function subscriptionPayloadFromStripe(subscription, existing = {}, fallback = {}) {
  ...
  const auditLimit = auditLimitForStatus(status);
  ...
  const shouldResetUsage = !existing?.id || periodChanged || becamePaid || limitChanged;
