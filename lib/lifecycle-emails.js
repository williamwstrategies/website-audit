...
const abandonedCartSteps = [
  {
    ...
    previewText: 'Your PitchProof checkout is still waiting.',
    headline: 'Your PitchProof checkout is still waiting.',
    paragraphs: [
      'You started choosing a PitchProof plan, but checkout was not completed.',
      'If you were trying to unlock the full report, PDF exports, saved reports, monthly scans, or white-label presentation, you can come back and finish setup now.',
...
const sevenDayTrialBroadcastStep = {
  ...
  previewText: "We've unlocked a 7-day free trial on your PitchProof account.",
...
function ctaUrlForStep(step = {}) {
  ...
  if (step.template === 'abandoned_cart' || step.campaign === ABANDONED_CART_CAMPAIGN_KEY) return billingUrl();
    ...
    <div style="margin:0;padding:28px;background:#f5f5f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1d1d1f;">
      ...
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e5ea;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(0,0,0,0.08);">
        ...
        <div style="padding:28px 30px;">
          ...
          <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3d3d43;">We've unlocked a <strong style="color:#1d1d1f;">7-day free trial</strong> on your PitchProof account.</p>
...
function sevenDayTrialEmailText({ user, profile, step }) {
  ...
  return [
    name ? `Hi ${name},` : 'Hi,',
    '',
    "We've unlocked a 7-day free trial on your PitchProof account.",
    '',
  ...
  for (const candidate of selected) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[LeadCheck] Abandoned cart email failure could not be recorded:', recordError?.message || recordError);
      }
      failed.push({
        email: candidate.user.email,
        user_id: candidate.user.id,
