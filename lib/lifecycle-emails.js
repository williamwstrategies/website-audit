...
function ctaUrlForStep(step = {}) {
  ...
  if (step.template === 'abandoned_cart' || step.campaign === ABANDONED_CART_CAMPAIGN_KEY) return abandonedCartOfferUrl();
...
async function runAbandonedSignupCampaign(options = {}) {
  ...
  if (!billing.abandonedCartDiscountConfigured() && !dryRun) {
    return {
      ...
      would_send: 0,
      failed: 0,
      skipped: { abandoned_cart_discount_not_configured: true },
      deliveries: [],
      failures: [],
    };
  }
