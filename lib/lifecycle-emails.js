...
function emailHtml({ user, profile, step }) {
  ...
  const accountLink = freePreviewUrl();
...
function emailText({ user, profile, step }) {
  ...
  return [
    ...
    '',
    `${step.cta}: ${freePreviewUrl()}`,
    `Your account: ${freePreviewUrl()}`,
    `Website: ${websiteUrl()}`,
    '',
...
async function updateEmailEvent({ user, step, status, providerId, errorMessage, metadata = {} }) {
  const rows = await supabaseRest('lifecycle_email_events', {
    method: 'PATCH',
    query: {
...
async function recordEmailEvent({ user, step, status = 'sent', providerId = '', errorMessage = '', metadata = {} }) {
  ...
  try {
    ...
    if (error?.details?.code === '23505') {
      const existing = await existingEmailEvent(user.email, step.key);
