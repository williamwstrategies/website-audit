...
const CLIENT_LIST_ACQUISITION_BROADCAST_STEP_KEY = 'client_list_acquisition_2026_09_23';
...
const FIRST_MONTH_DISCOUNT_COUPON_CODE = '50OFF1STMONTH';
const FIRST_MONTH_DISCOUNT_DEFAULT_START_AT = '2026-08-22T11:00:00-04:00';
...
const abandonedCartSteps = [
  {
    ...
    previewText: 'Come back and finish choosing your PitchProof plan.',
    headline: 'Your PitchProof checkout is still waiting.',
    paragraphs: [
      'You started choosing a PitchProof plan, but checkout was not completed.',
      'Come back through this link to finish checkout and start generating client-ready website reports.',
...
const incompleteAccountOfferSteps = [
  {
    key: 'incomplete_account_plan_reminder_1h',
    legacyKeys: ['first_month_50_after_signup'],
    delayMinutes: 60,
    subject: 'Finish setting up PitchProof',
    previewText: 'Your PitchProof account is ready when you are.',
    headline: 'Your PitchProof account is ready.',
    paragraphs: [
      'You created a PitchProof account, but setup was not completed.',
      'If you still want to generate client-ready website reports, come back through this link and choose the plan that fits your agency.',
...
const sevenDayTrialBroadcastStep = {
  ...
  subject: 'Try PitchProof for $1',
  previewText: 'Get full PitchProof access for 7 days for just $1.',
  headline: 'Get 7 days of PitchProof for $1.',
...
const firstMonthDiscountSequenceSteps = [
  {
    ...
    couponCode: FIRST_MONTH_DISCOUNT_COUPON_CODE,
  },
];
...
function trialBroadcastHeaderImageUrl() {
  return `${appUrl()}/assets/pitchproof-1-dollar-trial-email.jpg`;
}
...
function firstMonthDiscountOfferUrl() {
  ...
  url.searchParams.set('source', 'first_month_discount_sequence');
  return url.toString();
}
...
function aiVisibilityBroadcastStepForSubscription(subscription) {
  ...
  return {
    ...
    previewText: 'AI Visibility Reports are coming soon. Choose a plan now so you do not miss the rollout.',
    cta: 'Choose a Plan',
      ...
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e5ea;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(0,0,0,0.08);">
        ...
        <div style="padding:28px 30px;">
          ...
          <div style="margin:0 0 22px;padding:16px 18px;border:1px solid #f0d47a;border-radius:14px;background:#fff8df;color:#3d3d43;font-size:14px;line-height:1.65;">
            ...
            <p style="margin:0;">Choose a plan now and be ready when AI Visibility Reports roll out.</p>
          </div>
          ` : ''}
...
function aiVisibilityComingSoonEmailText({ user, profile, step }) {
  ...
  if (showOffer) {
    lines.push(
      ...
      '',
      'Choose a plan now and be ready when AI Visibility Reports roll out.',
      ''
    );
  }
...
function firstMonthDiscountEmailHtml({ user, profile, step }) {
  ...
  const couponCode = cleanText(step.couponCode || FIRST_MONTH_DISCOUNT_COUPON_CODE);
      ...
      <div style="max-width:660px;margin:0 auto;background:#ffffff;border:1px solid #e5e5ea;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(0,0,0,0.08);">
        ...
        <div style="padding:0 30px 30px;">
          ...
          <div style="margin:0 0 20px;padding:16px 18px;border:1px solid #f0d47a;border-radius:14px;background:#fff8df;color:#3d3d43;font-size:14px;line-height:1.65;">
            ...
            <p style="margin:0;">Use coupon code <strong style="color:#1d1d1f;">${htmlEscape(couponCode)}</strong> at checkout. This offer is only available during this final window.</p>
          </div>
          ${bulletHtml}
          <a href="${htmlEscape(accountLink)}" style="display:inline-block;padding:13px 18px;border-radius:12px;background:#f5c842;color:#1d1d1f;text-decoration:none;font-weight:800;">${htmlEscape(step.cta)}</a>
  ...
  return `
    <style>
      @media only screen and (max-width: 640px) {
        .pp-email-shell { padding:0 !important; background:#ffffff !important; }
        .pp-email-card { width:100% !important; max-width:100% !important; border-left:0 !important; border-right:0 !important; border-radius:0 !important; box-shadow:none !important; }
        .pp-email-image { width:100% !important; max-width:100% !important; height:auto !important; }
        .pp-email-block { padding-left:20px !important; padding-right:20px !important; }
      }
    </style>
    <div class="pp-email-shell" style="margin:0;padding:28px;background:#f5f5f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1d1d1f;">
      <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">${htmlEscape(step.previewText)}</div>
      <div class="pp-email-card" style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e5ea;border-radius:18px;overflow:hidden;box-shadow:0 18px 48px rgba(0,0,0,0.08);">
        <a href="${htmlEscape(accountLink)}" style="display:block;text-decoration:none;">
          <img class="pp-email-image" src="${htmlEscape(trialBroadcastHeaderImageUrl())}" alt="Try PitchProof for $1. Get full access for 7 days for just $1." width="640" style="display:block;width:100%;max-width:640px;height:auto;border:0;">
        </a>
        <div class="pp-email-block" style="padding:28px 30px;border-bottom:1px solid #ececf0;">
          <p style="margin:0 0 10px;color:#8a6312;text-transform:uppercase;letter-spacing:.08em;font-size:12px;font-weight:800;">PitchProof</p>
          <h1 style="margin:0;color:#1d1d1f;font-size:26px;line-height:1.2;letter-spacing:0;">${htmlEscape(step.headline)}</h1>
        </div>
        <div class="pp-email-block" style="padding:28px 30px;">
          <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3d3d43;">${greeting}</p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3d3d43;">For the next 7 days, you can try PitchProof for just <strong style="color:#1d1d1f;">$1</strong>.</p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3d3d43;">PitchProof helps agencies turn a prospect's website into a client-ready sales report, showing what's hurting trust, what's hurting conversions, and what should be fixed first.</p>
          ...
          <p style="margin:0 0 24px;font-size:15px;line-height:1.65;color:#3d3d43;">That's where PitchProof is most useful: it gives you a reason to reach out, a clear way to explain the opportunity, and a professional report to back up the conversation.</p>
          <a href="${htmlEscape(accountLink)}" style="display:inline-block;padding:13px 18px;border-radius:12px;background:#ef1d1d;color:#ffffff;text-decoration:none;font-weight:800;">${htmlEscape(step.cta)}</a>
          <p style="margin:18px 0 0;font-size:12px;line-height:1.55;color:#777;">Stripe charges $1 today to activate access. After 7 days, your selected plan continues at the normal monthly price unless you cancel.</p>
        </div>
        <div class="pp-email-block" style="padding:18px 30px;background:#fafafa;border-top:1px solid #ececf0;font-size:12px;line-height:1.6;color:#777;">
          <a href="${htmlEscape(unsubscribe)}" style="color:#6e6e73;">Unsubscribe from PitchProof emails</a>
        </div>
      </div>
    </div>
...
function sevenDayTrialEmailText({ user, profile, step }) {
  ...
  return [
    name ? `Hi ${name},` : 'Hi,',
    '',
    'For the next 7 days, you can try PitchProof for just $1.',
    '',
    "PitchProof helps agencies turn a prospect's website into a client-ready sales report, showing what's hurting trust, what's hurting conversions, and what should be fixed first.",
    '',
    ...
    '',
    'That is where PitchProof is most useful: it gives you a reason to reach out, a clear way to explain the opportunity, and a professional report to back up the conversation.',
    '',
    `${step.cta}: ${ctaUrlForStep(step)}`,
    '',
    'Stripe charges $1 today to activate access. After 7 days, your selected plan continues at the normal monthly price unless you cancel.',
    '',
...
async function runFirstMonthDiscountSequenceBroadcast(options = {}) {
  ...
  const { deliveries, failures } = processedStep
    ? await deliverBroadcastStep({
        ...
        step: processedStep,
        campaign,
        source: 'first_month_discount_sequence',
        ctaUrl,
      })
    : { deliveries: [], failures: [] };
  ...
  return {
    ...
    cta_url: ctaUrl,
    coupon_code: FIRST_MONTH_DISCOUNT_COUPON_CODE,
