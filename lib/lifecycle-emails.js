  ...
  for (const candidate of selected) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[LeadCheck] Incomplete account offer failure could not be recorded:', recordError?.message || recordError);
      }
      failed.push({
        email: candidate.user.email,
        user_id: candidate.user.id,
  ...
  for (const recipient of selected) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[LeadCheck] AI Visibility broadcast failure could not be recorded:', recordError?.message || recordError);
      }
      failures.push({
        email: recipient.user.email,
        user_id: recipient.user.id,
  ...
  for (const recipient of selected) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[LeadCheck] Client list acquisition broadcast failure could not be recorded:', recordError?.message || recordError);
      }
      failures.push({
        email: recipient.user.email,
        user_id: recipient.user.id,
