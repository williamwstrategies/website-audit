...
function isProviderPaymentRequired(responseStatus, providerStatusCode, providerStatusMessage = '') {
  return responseStatus === 402
    || providerStatusCode === 40200
    || providerStatusCode === 40201
    || /payment required|insufficient|balance|funds|billing/i.test(providerStatusMessage);
}
...
async function searchBusinessListings(input = {}) {
  ...
  if (!response.ok || providerStatusCode !== 20000) {
    const paymentRequired = isProviderPaymentRequired(response.status, providerStatusCode, providerStatusMessage);
    throw httpError(
      paymentRequired ? 402 : (response.ok ? 502 : response.status),
      paymentRequired
        ? 'DataForSEO Business Listings returned payment required. Add account balance or enable paid API access in DataForSEO, then retry Lead Finder.'
        : providerStatusMessage,
      paymentRequired ? 'business_search_provider_payment_required' : 'business_search_provider_error',
