...
function analyzePromptResponse({ intent, prompt, response, business }) {
  ...
  if (!responseText) {
    return {
      ...
      cost: round(providerResponse.cost || providerResponse.moneySpent, 6),
      durationMs: Number(providerResponse.durationMs) || 0,
      error: 'DataForSEO returned a successful task without usable response text.',
      ...
      statusMessage: cleanText(providerResponse.statusMessage, 500),
      responseTextLength: 0,
      citationCount: asArray(providerResponse.citedUrls).length,
      fanOutQueryCount: asArray(providerResponse.fanOutQueries).length,
    };
  }
...
async function runAiVisibilityAssessment(input = {}, options = {}) {
  ...
  console.log('[AI Visibility] assessment started', {
    mockMode: mock,
    promptsPlanned: prompts.length,
    countryCode,
    city,
  ...
  console.log('[AI Visibility] assessment result', {
    status: assessment.status,
    score: assessment.score,
    label: assessment.label,
    promptsTested: assessment.promptsTested,
    successfulRequests: assessment.successfulRequests,
    failedRequests: assessment.failedRequests,
    mentions: assessment.mentions,
