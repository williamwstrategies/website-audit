...
const { normalizeAiVisibility } = require('./ai-visibility');
...
function categoryLabel(key, label) {
  const normalized = `${String(key || '')} ${asText(label)}`.toLowerCase();
  const fallback = asText(label) || titleCase(key);
  if (normalized.includes('website') && normalized.includes('health')) return 'Website Health';
  if (normalized.includes('lead') || normalized.includes('conversion')) return 'Conversion';
  if (normalized.includes('trust') || normalized.includes('local') || normalized.includes('credibility')) return 'Trust & Credibility';
  if (normalized.includes('seo') || normalized.includes('visibility')) return 'SEO';
  if (normalized.includes('technical')) return 'Technical';
  if (normalized.includes('performance') || normalized.includes('speed')) return 'Performance';
  if (normalized.includes('visual') || normalized.includes('design') || normalized.includes('user')) return 'Design';
  return fallback || 'Website Performance';
}

function normalizedCategoryMax(score, max) {
...
function categoryRows(payload) {
  ...
  return objectEntries(payload.categories).map(([key, rawCategory]) => {
    ...
    const max = normalizedCategoryMax(score, category.max);
    const label = categoryLabel(key, category.label);
...
function recommendationRows(payload) {
  return asArray(payload.recommendations).map((rawRecommendation, index) => {
    ...
    return {
      ...
      priority: asText(recommendation.priority) || (index < 3 ? 'High' : 'Medium'),
    };
  });
}
...
function aiVisibilityPayload(payload) {
  const raw = asRecord(firstDefinedValue(payload.aiVisibility, payload.ai_visibility));
  if (!objectEntries(raw).length) return null;
  return normalizeAiVisibility(raw);
}
...
async function formatReportForPrint(report, branding = {}, options = {}) {
  const payload = reportPayload(report);
  const context = reportContext(report);
  ...
  const aiVisibility = aiVisibilityPayload(payload);
  ...
  const poweredBy = options.showPoweredBy ? cleanText(branding.powered_by) || 'Powered by PitchProof' : '';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .pill { display: inline-flex; align-items: center; border-radius: 999px; padding: 4px 8px; font-size: 10px; font-weight: 700; background: #fff4cf; color: ${secondary}; }
  ...
  .ai-grid { display: grid; grid-template-columns: .75fr 1.25fr; gap: 12px; }
  .ai-score-box, .ai-metric, .ai-actions { border: 1px solid #eeeeef; border-radius: 12px; padding: 12px; background: #fafafa; }
  .ai-score { font-size: 30px; font-weight: 800; line-height: 1; }
  .ai-label { display: inline-flex; margin-top: 8px; border-radius: 999px; padding: 4px 8px; background: #fff4cf; color: ${secondary}; font-size: 10px; font-weight: 800; }
  .ai-metrics { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 10px; }
  .ai-metric span { display: block; color: #86868b; font-size: 9px; font-weight: 800; letter-spacing: .06em; text-transform: uppercase; }
  .ai-metric strong { display: block; margin-top: 4px; font-size: 13px; }
  .ai-actions { margin-top: 10px; }
  .ai-actions ul { margin: 6px 0 0; padding-left: 16px; }
  .ai-disclaimer { margin-top: 8px; color: #86868b; font-size: 10px; }
  ...
  .priority-chip { display: inline-flex; align-items: center; margin-top: 8px; border-radius: 999px; padding: 4px 8px; font-size: 10px; font-weight: 800; }
  .priority-chip.high { border: 1px solid rgba(180, 35, 24, .24); background: #fff4f1; color: #b42318; }
  .priority-chip.medium { border: 1px solid rgba(167, 111, 20, .24); background: #fff8e2; color: #a76f14; }
  .priority-chip.low { border: 1px solid rgba(31, 143, 95, .22); background: #f1f8f3; color: #1f8f5f; }
...
<body>
  <main class="document">
    ...
    ${aiVisibility ? `<section class="section">
      <div class="section-head"><h2>AI Visibility <span class="pill">Beta</span></h2><span class="pill">${escHtml(aiVisibility.status === 'complete' ? aiVisibility.label : 'Unavailable')}</span></div>
      ${aiVisibility.status === 'complete' ? `
        <div class="ai-grid">
          <div class="ai-score-box">
            <span class="label">AI Visibility Score</span>
            <div class="ai-score" style="color:${colorForScore(Number(aiVisibility.score || 0), Number(aiVisibility.maxScore || 10))};">${escHtml(Number(aiVisibility.score || 0).toFixed(1))}<span style="font-size:14px;color:#86868b;"> / ${escHtml(aiVisibility.maxScore || 10)}</span></div>
            <span class="ai-label">${escHtml(aiVisibility.label)}</span>
          </div>
          <div class="ai-score-box">
            <span class="label">What This Means</span>
            <p>${escHtml(aiVisibility.summary)}</p>
          </div>
        </div>
        <div class="ai-metrics">
          <div class="ai-metric"><span>Mentions</span><strong>${escHtml(aiVisibility.mentions)} / ${escHtml(aiVisibility.promptsTested)}</strong></div>
          <div class="ai-metric"><span>Recommendations</span><strong>${escHtml(aiVisibility.recommendations)} / ${escHtml(aiVisibility.promptsTested)}</strong></div>
          <div class="ai-metric"><span>Citations</span><strong>${escHtml(aiVisibility.citations)} / ${escHtml(aiVisibility.promptsTested)}</strong></div>
          <div class="ai-metric"><span>Prompts Tested</span><strong>${escHtml(aiVisibility.promptsTested)}</strong></div>
        </div>
        ${aiVisibility.opportunities?.length ? `<div class="ai-actions"><strong>How to improve AI visibility</strong><ul>${aiVisibility.opportunities.map(item => `<li>${escHtml(item)}</li>`).join('')}</ul></div>` : ''}
      ` : `<p class="muted">${escHtml(aiVisibility.summary || 'AI visibility data unavailable for this scan.')}</p>`}
      <div class="ai-disclaimer">${escHtml(aiVisibility.disclaimer)}</div>
    </section>` : ''}
      ...
      <div class="list">
        ${findings.map(finding => `
          <article class="item">
            ...
            <div class="priority-chip ${escHtml(priorityTone(finding.priority))}">${escHtml(finding.priority)} priority</div>
          </article>
      ...
      <div class="list">
        ${recommendations.map(recommendation => `
          <article class="item">
            ...
            <div class="priority-chip ${escHtml(priorityTone(recommendation.priority))}">${escHtml(recommendation.priority)} priority</div>
    ...
    <footer class="footer-row">
      <div>
        ...
        ${poweredBy ? `<div class="muted">${escHtml(poweredBy)}</div>` : ''}
      </div>
...
async function generateReportPdf({ report, branding, showPoweredBy = false }) {
  const html = await formatReportForPrint(report, branding, { showPoweredBy });
  ...
  try {
    ...
    const buffer = await page.pdf({
      ...
      footerTemplate: `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:8px;color:#86868b;width:100%;padding:0 0.5in;display:flex;justify-content:space-between;"><span>${showPoweredBy ? 'Powered by PitchProof' : 'Confidential assessment'}</span><span><span class="pageNumber"></span> / <span class="totalPages"></span></span></div>`,
