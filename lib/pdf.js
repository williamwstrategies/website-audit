...
async function formatReportForPrint(report, branding = {}, options = {}) {
  ...
  const poweredBy = options.showPoweredBy ? cleanText(branding.powered_by) || 'Powered by Website Strategy Scan' : '';
...
async function generateReportPdf({ report, branding, showPoweredBy = false }) {
  ...
  try {
    ...
    const buffer = await page.pdf({
      ...
      footerTemplate: `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:8px;color:#86868b;width:100%;padding:0 0.5in;display:flex;justify-content:space-between;"><span>${showPoweredBy ? 'Powered by Website Strategy Scan' : 'Confidential assessment'}</span><span><span class="pageNumber"></span> / <span class="totalPages"></span></span></div>`,
